...
test("option contract pagination consumes every Alpaca page when no result cap is requested", async () => {
  await withMockedFetch([
    ...
    assert.ok(contracts.every((row) => Number.isFinite(Date.parse(String(row.retrievedAt)))));
...
test("OPRA option-chain pagination consumes all pages, retains provenance, and deduplicates contracts", async () => {
  await withMockedFetch([
    ...
    assert.equal(call.effectiveFeed, null);
    assert.equal(call.validationBasis, "request_feed_opra");
...
test("OPRA option-chain retains an observed matching response feed", async () => {
  await withMockedFetch([{
    body: {
      feed: "OPRA",
      snapshots: {
    ...
    requestId: "chain-request-observed-feed"
  }], async () => {
    const result = await fetchOptionChainSnapshots("SPY", { feed: "opra" });
    assert.equal(result.snapshots[0]?.requestedFeed, "opra");
    assert.equal(result.snapshots[0]?.effectiveFeed, "opra");
    assert.equal(result.snapshots[0]?.validationBasis, "request_feed_opra");
  });
});
