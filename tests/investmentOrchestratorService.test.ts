...
import {
  runInvestmentOrchestrator,
  type InvestmentStrategyLane
} from "../src/services/investmentOrchestratorService.js";
...
const orderedLanes: readonly WorkstreamLane[] = [
  "equity",
  "options_0dte",
  "options_leaps"
];
    ...
    lanes: [
      {
        ...
        execute: async (shared) => {
          invocations.push("equity");
          receivedContexts.push(shared);
      ...
      },
      {
        ...
        enabled: true,
        execute: async (shared) => {
          invocations.push("options_leaps");
          receivedContexts.push(shared);
...
for (const scenario of [
  {
    name: "equity failure",
    lane: "equity" as const,
    message: "EQUITY_EVALUATION_FAILED"
  },
  {
    name: "0DTE failure",
    lane: "options_0dte" as const,
    message: "ZERO_DTE_EVALUATION_FAILED"
  },
  {
    name: "lane timeout",
    lane: "options_0dte" as const,
    message: "WORKSTREAM_TIMEOUT"
  }
]) {
  test(`${scenario.name} does not suppress later enabled lanes or successful proposals`, async () => {
    const invocations: WorkstreamLane[] = [];
    const lanes: InvestmentStrategyLane<SharedContext, Proposal>[] = orderedLanes.map(
      (lane) => ({
        lane,
        enabled: true,
        execute: async () => {
          invocations.push(lane);
          if (lane === scenario.lane) throw new Error(scenario.message);
    ...
    assert.equal(
      result.workstreamResults.find(({ lane }) => lane === scenario.lane)?.outcome,
      "error"
    );
    assert.equal(
      result.proposals.some(({ id }) => id === `${scenario.lane}-proposal`),
      false
    );
    assert.equal(
...
test("no_action and bounded readiness results preserve proposals from later lanes", async () => {
    ...
    lanes: [
      {
        ...
        execute: async () => {
          invocations.push("equity");
          return { proposals: [], reason_codes: ["NO_ELIGIBLE_POSTGRES_CANDIDATES"] };
        }
      },
      {
        lane: "options_0dte",
        enabled: true,
        execute: async () => {
          invocations.push("options_0dte");
          return { proposals: [], reason_codes: ["POSTGRES_STOCK_SNAPSHOT_STALE"] };
        }
      },
      {
        lane: "options_leaps",
        ...
        execute: async () => {
          invocations.push("options_leaps");
          return { proposals: [{ id: "preserved-leaps-proposal" }] };
        }
      }
    ]
  });
  ...
  assert.deepEqual(invocations, orderedLanes);
  assert.deepEqual(
    result.workstreamResults.map(({ outcome }) => outcome),
    ["no_action", "no_action", "success"]
  );
  assert.deepEqual(result.workstreamResults[1]?.reason_codes, [
...
test("shared pre-lane failure remains fatal and does not fabricate lane results", async () => {
  ...
  await assert.rejects(
    runInvestmentOrchestrator<SharedContext, Proposal>({
      ...
      loadSharedContext: async () => {
        throw new Error("POSTGRES_SHARED_CONTEXT_UNAVAILABLE");
      },
      lanes: [{
        lane: "equity",
        enabled: true,
