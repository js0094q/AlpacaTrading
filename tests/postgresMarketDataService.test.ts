...
const withEvidenceFingerprints = <T extends Record<string, unknown>>(
  rows: readonly T[]
): Array<T & { evidenceFingerprint: string }> => rows.map((row) => ({
  ...row,
  evidenceFingerprint: optionSnapshotEvidenceFingerprint(row as unknown as PostgresOptionSnapshot)
}));
...
test("refresh persists genuine SIP and OPRA evidence in PostgreSQL", async () => {
  ...
  ].map((name) => [name, async (rows: Record<string, unknown>[]) => {
