...
test("successful OPRA request provenance validates feed without fabricating an observed feed", async () => {
  ...
  const feature = result.features.at(-1)!.features;
  assert.equal(feature.optionFeedValidated, true);
  assert.equal(feature.optionFeedValidationBasis, "request_feed_opra");
  const classifications = feature.optionFieldClassifications as Record<string, string>;
  assert.equal(classifications.feedValidationBasis, "execution_gate");
});

test("a current option observation retrieved after the stock snapshot reaches decision features", async () => {
  const result = await buildOptionFeaturesFixture({
    contracts: [optionContractFixture("SPY260829C00560000", 560)],
    snapshots: [optionSnapshotFixture("SPY260829C00560000", {
      observedAt: "2026-06-29T20:00:02.000Z",
      quoteTimestamp: "2026-06-29T20:00:02.000Z",
      snapshotTimestamp: "2026-06-29T20:00:02.000Z",
      retrievedAt: "2026-06-29T20:00:03.000Z"
    })]
  });
  assert.equal(result.features.at(-1)!.features.optionContractEligible, true);
  assert.equal(result.features.at(-1)!.features.optionDelta, 0.5);
  assert.equal(result.targets[0]?.asOf, "2026-06-29T20:00:02.000Z");
});
...
test("stale or missing option liquidity evidence fails eligibility without fabricating values", async () => {
  ...
  const decisionInputs = result.targets[0]!.optionsStrategy?.decisionInputs as Record<string, unknown>;
  assert.equal(
...
test("missing implied volatility preserves existing option eligibility and baseline provenance", async () => {
  const result = await buildPostgresFeaturesAndTargets({
    bars: bars(60), stockSnapshots: [stockSnapshot()],
    optionContracts: [optionContractFixture("SPY260829C00560000", 560)],
    optionSnapshots: [optionSnapshotFixture("SPY260829C00560000", { impliedVolatility: null })],
    riskProfile: "aggressive", optionsEnabled: true,
    repository: { upsertFeatureSnapshots: async (rows: unknown[]) => ({ stored: rows.length }), upsertTargetSnapshots: async (rows: unknown[]) => ({ stored: rows.length }) } as never,
    context
  });
  ...
  assert.equal(feature.atmImpliedVol, null);
  assert.equal(feature.optionContractEligible, true);
  assert.equal(result.targets[0]!.optionsStrategy?.optionsCandidate !== null, true);
  const decisionInputs = result.targets[0]!.optionsStrategy?.decisionInputs as Record<string, unknown>;
  assert.equal(decisionInputs.impliedVolatility, null);
  assert.equal(decisionInputs.volatilityAdjustmentSource, "existing_strategy_baseline");
});
