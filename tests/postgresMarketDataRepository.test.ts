...
test("market-data writes are fenced PostgreSQL upserts with source provenance", async () => {
  ...
  const contractEvidence = JSON.parse(String(contractWrite.values?.[10]));
  assert.equal(contractEvidence.contractId, "contract-spy");
  assert.equal(contractEvidence.status, "active");
  assert.equal(contractEvidence.openInterest, 1_000);
  const snapshotWrite = writes.find((entry) => entry.text.includes("INSERT INTO option_snapshots"))!;
  const snapshotEvidence = JSON.parse(String(snapshotWrite.values?.[20]));
    ...
    query: async (text: string) => {
      ...
      if (text.includes("FROM option_snapshots")) {
        return { rows: [{
          ...
          evidence: { underlyingPrice: 744.36, bidSize: 10, askSize: 12, spread: 0.1, spreadPct: 0.031746, freshnessStatus: "fresh", requestedFeed: "opra", effectiveFeed: "opra", endpoint: "/v1beta1/options/snapshots/SPY", pageToken: "page-2", retrievedAt: "2026-07-21T13:42:00.000Z" },
