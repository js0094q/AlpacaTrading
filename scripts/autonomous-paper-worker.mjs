...
const EXPECTED_DEFERRED_REASON_PATTERN = /\b(POSTGRES_STOCK_SNAPSHOT_STALE|POSTGRES_REVIEW_MARKET_EVIDENCE_STALE|POSTGRES_OPTION_SNAPSHOTS_CURRENT_MISSING|POSTGRES_SELECTED_OPTION_EVIDENCE_REFRESH_INCOMPLETE|POSTGRES_DECISION_MARKET_SESSION_INELIGIBLE|NO_ELIGIBLE_POSTGRES_CANDIDATES|NO_READY_POSTGRES_ORDER_INTENTS)\b/;
...
const WORKSTREAMS = [
  ...
  ["zero-dte:reconcile", ["--format=json"]],
  ["paper:options:discover", ["--lane=options_leaps", `--maxCandidates=${PAPER_EXPLORATION_MAX_CANDIDATES}`, "--format=json"]],
  ["paper:execute:reviewed", ["--confirmPaper", "--sections=optionBuys", "--strategyFamily=leaps", "--format=json"]],
  ["research:daily", ["--riskProfile=aggressive", "--optionsEnabled=true", "--excludeZeroDte=true", "--excludeLeaps=true", `--maxCandidates=${PAPER_EXPLORATION_MAX_CANDIDATES}`, "--assetClass=all", "--format=json"]],
...
const classify = ({ exitCode, output, spawnError, timedOut }, script) => {
  ...
  if (exitCode === 0) {
    ...
    if (
      /"status"\s*:\s*"deferred"/i.test(output) &&
      reasonCode &&
      EXPECTED_DEFERRED_REASON_PATTERN.test(reasonCode)
    ) {
      return {
