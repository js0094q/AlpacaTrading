...
export const selectExpression = (input: CandidateInput): StrategySelectorResult => {
  ...
  const isAggressiveAllowed = config.enableAggressivePaperStrategies;
  ...
  if (input.hasOptionsData && hasStrongLiquidity && isAggressiveAllowed) {
    if (Math.abs(input.confidence) >= 0.8 && percent(volatility, input.direction === "long" ? input.atr ?? 0 : input.atr ?? 0) > 0) {
  ...
  if (!rationale.length) {
    rationale.push(
      `Direction ${input.direction} has insufficient options-structure edge; shares are default`
    );
  }

  const optionScore = input.liquidityScore;
  const optionsCandidate = hasStrongLiquidity
    ? {
        ...
        estimatedEntryPrice: optionScore,
        maxLoss: null,
        maxProfit: null,
        breakeven: null,
        liquidityScore: optionScore
      }
    : undefined;
