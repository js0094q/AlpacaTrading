...
import {
  ...
  type StrategySelectionThresholds
...
export const selectExpressionWithPolicy = (
  ...
  thresholdOverrides: Partial<StrategySelectionThresholds> = {}
): StrategySelectorResult => {
  const thresholds = { ...BASELINE_DECISION_THRESHOLDS, ...thresholdOverrides };
  ...
  if (
    ...
    input.confidence < thresholds.minimumDirectionalConfidence
  ) {
  ...
  const hasGoodSpread = input.spreadPct === null
    ? true
    : input.spreadPct <= thresholds.maximumOptionSpreadPct;
  const hasStrongLiquidity = input.liquidityScore >= thresholds.minimumOptionLiquidityScore;
  ...
  if (input.hasOptionsData && hasStrongLiquidity) {
    if (
      longBias &&
      input.confidence >= thresholds.minimumLongOptionConfidence &&
      expectedMagnitude >= thresholds.minimumOptionExpectedReturnPct
    ) {
      if (
        isAggressiveAllowed &&
        iv > 0.25 &&
        hasGoodSpread &&
        input.confidence > thresholds.minimumAggressiveOptionConfidence
      ) {
        preferred = "long_call";
  ...
  if (input.hasOptionsData && hasStrongLiquidity && isAggressiveAllowed) {
    if (
      Math.abs(input.confidence) >= thresholds.minimumDefinedRiskConfidence &&
      percent(volatility, input.atr ?? 0) > 0
    ) {
      if (longBias && expectedMagnitude >= thresholds.minimumDefinedRiskExpectedReturnPct) {
      ...
      if (shortBias && expectedMagnitude <= -thresholds.minimumDefinedRiskExpectedReturnPct) {
