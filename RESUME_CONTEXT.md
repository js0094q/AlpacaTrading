...
- Migration `010_lane_aware_target_identity.sql` changes target and option
  strategy persistence from the former three-column identity to additive
  `strategy_family` and `expression_id` columns plus the full five-column key.
- Before any future production migration, stop the paper service so no runtime
  writer overlaps it. The exact disposable-PostgreSQL verification command is
  `POSTGRES_INTEGRATION_TEST_ENABLED=true tsx --test tests/postgresNeonIntegration.test.ts`.
- Before production migration, rollback means code rollback. After migration,
  retain the additive columns and lane rows and deploy forward, because
  dropping lane rows would be destructive.
- Migration 010 has **not** been applied to the paper VPS. No paper service
  was restarted and no deployment occurred for this work.
...
- Market-data feed defaults are centralized and paper-safe:
  ...
  - options feed `opra`
- Optional stock streaming remains disabled by default (`ALPACA_STOCK_STREAM_ENABLED=false`). Section 6 centralizes stream ownership through one registry entry per endpoint/feed, authenticates once, fans out normalized paper events, retains last-known trade/quote/bar state, and reconnects with observable exponential delay capped by `ALPACA_STOCK_STREAM_RECONNECT_MAX_MS` (default 60000).
- `src/services/alpacaDataHubService.ts` is the cycle-scoped shared data source for equity, 0DTE, and LEAPS. The PostgreSQL-authoritative market refresh hydrates it once per cycle; repeated lane reads reuse the same quote object and do not repeat REST hydration. Feed, provider/source timestamps, receipt time, symbol, event type, and paper environment remain explicit.
- Reconciliation order updates and bounded account-activity polling feed the same normalized cache. Assignment, exercise, and expiration activities remain REST-polled because they are not assumed to arrive on the market-data stream. Polls do not overlap, and stream/consumer failures do not terminate unrelated lanes.
- `research:daily` owns the optional stream for the orchestrator cycle, starts it once, runs one non-overlapping account-activity poll alongside the shared refresh, and stops the socket in command cleanup. No dashboard stream owner or second per-lane socket is introduced. Set `ALPACA_STOCK_STREAM_ENABLED=true` plus the documented SIP URL and non-secret controls in the applicable runtime environment; do not edit the repository `.env`.
